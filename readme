...
2. Tag and push the image to Docker Hub:
   ```bash
   sudo docker tag my-web-app niha070/myweb:latest
   sudo docker login
   sudo docker push niha070/myweb:latest
   ```

### 3. Deploy on EC2
1. SSH into the EC2 instance and pull the Docker image:
   ```bash
   sudo docker pull niha070/myweb:latest
   sudo docker run -d -p 80:80 niha070/myweb:latest
   ```
...
## Outputs
- **Docker Image**: docker pull niha070/myweb:latest
## Conclusion
